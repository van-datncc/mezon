...
export const RecentInteractiveSearch = React.memo(({
    ...
    const generateChannelMatch = useCallback(
        (searchText: string, dataSource: DirectEntity[]) => {
            ...
            for (const channel of dataSource) {
                ...
                const hasUserMatch = channel?.usernames?.some?.((username: string) =>
            ...
            const childChannels = dataSource.filter((item) =>
                item?.parent_id && matchChannelIds.has(item.parent_id)
            );
    ...
    useEffect(() => {
        onSearchResults([
            ...((topUserSuggestionUser ? [topUserSuggestionUser] : []) as DirectEntity[]),
            ...listDMText,
            ...listChannelsText
        ]);
    }, []);
            ...
            <View style={styles.searchInput}>
                ...
                >
                    <TouchableOpacity
                        activeOpacity={0.7}
                        onPress={() => {
                            console.log('Filter button pressed, opening tooltip');
