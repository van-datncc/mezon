...
const UserProfile = React.memo(({ userId, user, onClose, checkAnonymous, message, showAction = true, showRole = true }: userProfileProps) => {
	...
	const { color } = useMixImageColor(userById?.clan_avatar || userById?.user?.avatar_url || user?.avatarSm || userProfile?.user?.avatar_url);
	...
	return (
		<View style={[styles.wrapper]}>
			<View style={[styles.backdrop, { backgroundColor: userById || user?.avatar_url || user?.avatarSm ? color : Colors.titleReset }]}>
				<View style={[styles.userAvatar]}>
					<MezonAvatar
						width={80}
						height={80}
						avatarUrl={userById ? userById.clan_avatar : user?.avatar_url || user?.avatarSm}
						username={userById ? userById.clan_nick : userById?.user?.username || user?.display_name}
