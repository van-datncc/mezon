...
import { selectDefaultNotificationCategory, selectDefaultNotificationClan, selectnotificatonSelected, useAppDispatch } from '@mezon/store-mobile';
import { notifiReactMessageActions, notificationSettingActions, selectCurrentChannelId, selectCurrentClanId, selectNotifiReactMessage } from '@mezon/store-mobile';
...
export default function NotificationSetting() {
  ...
  useEffect(() => {
		if (getNotificationChannelSelected?.notification_setting_type) {
			setDefaultNotifyName(getNotificationChannelSelected.notification_setting_type);
  ...
  const handleRadioBoxPress = (checked: boolean, id: number) => {
    const notifyOptionSelected =  radioBox.map(item =>
      item && {...item, isChecked: item.id === id},
    )
    ...
    if(notifyOptionSelected.length) {
      ...
      if ([ENotificationType.ALL_MESSAGE,
          ENotificationType.MENTION_MESSAGE,
          ENotificationType.NOTHING_MESSAGE,
        ].includes(notifyOptionSettingSelected?.value)) {
        const body = {
