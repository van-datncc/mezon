...
import { emojiSuggestionActions, selectMemberClanByUserId, useAppDispatch } from '@mezon/store-mobile';
...
const EmojiDetail = forwardRef(({ item, currentUserId, onSwipeOpen }: ServerDetailProps, ref: Ref<SwipeableMethods>) => {
	...
	const focusTextInput = () => {
		if (currentUserId !== item.creator_id) {
			return;
		}
		setIsFocused(true);
		if (textInputRef) {
	...
	const handleBlur = () => {
		...
		} else if (currentUserId !== item.creator_id && emojiName !== item.shortname?.split(':')?.join('')) {
			setEmojiName(item.shortname?.split(':')?.join(''));
			Toast.show({
				type: 'info',
	...
	return (
		<Swipeable ref={ref} onSwipeableWillOpen={onSwipeOpen} enabled={currentUserId === item.creator_id} renderRightActions={RightAction}>
			<Pressable style={styles.container} onPress={focusTextInput}>
				<View style={styles.emojiItem}>
					...
					<View style={styles.emojiName}>
						...
						<TextInput
							editable={currentUserId === item.creator_id}
							ref={textInputRef}
							onBlur={handleBlur}
							onFocus={handleFocus}
							numberOfLines={1}
