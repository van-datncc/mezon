...
export default function MezonMenuContextItem(
    { hasBorder, isHeader, icon, title, titleStyle, onPress, containerStyle, disabled }: IMezonMenuContextItemProps
) {
        ...
        <TouchableOpacity
            ...
            <View>
                <Text
                    numberOfLines={1}
                    ellipsizeMode="tail"
                    style={[styles.title, isHeader && styles.textHeader, titleStyle]}>{title}</Text>
                {icon}
            </View>
        </TouchableOpacity>
    )
