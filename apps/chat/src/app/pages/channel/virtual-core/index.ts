...
export const observeElementOffset = <T extends Element>(instance: Virtualizer<T, any>, cb: ObserveOffsetCallBack) => {
	const element = instance.scrollElement;
	if (!element) {
		return;
	}
...
export const observeWindowOffset = (instance: Virtualizer<Window, any>, cb: ObserveOffsetCallBack) => {
	const element = instance.scrollElement;
	if (!element) {
		return;
	}
...
export class Virtualizer<TScrollElement extends Element | Window, TItemElement extends Element> {
	...
	private maybeNotify = memo(
		...
		},
		{
			...
			initialDeps: [this.isScrolling, this.range ? this.range.startIndex : null, this.range ? this.range.endIndex : null] as [
				boolean,
				number | null,
				number | null
			]
	...
	getVirtualItemForOffset = (offset: number) => {
		...
		return notUndefined(
			measurements[findNearestBinarySearch(0, measurements.length - 1, (index: number) => notUndefined(measurements[index]).start, offset)]
		);
	};
...
function calculateRange({ measurements, outerSize, scrollOffset }: { measurements: Array<VirtualItem>; outerSize: number; scrollOffset: number }) {
