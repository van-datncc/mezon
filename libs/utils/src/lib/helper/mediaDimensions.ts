...
export function calculateInlineImageDimensions(
	...
	const { width, height } = (getPhotoInlineDimensions(photo) as any) || DEFAULT_MEDIA_DIMENSIONS;
...
export function getStickerDimensions(sticker: ApiSticker, isMobile?: boolean): ApiDimensions {
	...
	const baseWidth = REM * (isMobile ? STICKER_SIZE_INLINE_MOBILE_FACTOR : STICKER_SIZE_INLINE_DESKTOP_FACTOR);
...
export function calculateMediaViewerDimensions({ width, height }: ApiDimensions, withFooter: boolean, isVideo = false): ApiDimensions {
...
export function calculateDimensions(availableWidth: number, availableHeight: number, mediaWidth: number, mediaHeight: number): ApiDimensions {
