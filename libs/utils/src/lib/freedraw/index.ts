...
export default class CanvasFreeDrawing {
	...
	constructor(params: CanvasFreeDrawingParameters) {
		...
		this.requiredParam(params, 'canvas');
		this.requiredParam(params, 'canvasCtx');
		...
		// initialize events
		this.events = {
			redrawEvent: new Event('redraw'),
			fillEvent: new Event('fill'),
	...
	addListeners(): void {
		this.listenersList.forEach((event) => {
			this.canvas.addEventListener(event.toLowerCase(), this.bindings[event] as EventListenerObject);
		});
	...
	removeListeners(): void {
		this.listenersList.forEach((event) => {
			this.canvas.removeEventListener(event.toLowerCase(), this.bindings[event] as EventListenerObject);
		});
	...
	isNodeColorEqual(i: Color, j: Color, t: number): boolean {
		...
		if (Object.prototype.hasOwnProperty.call(this.isNodeColorEqualCache, color1 + color2 + t)) {
