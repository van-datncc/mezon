...
const MessageAvatar = ({ user, message, isCombine, isReply }: IMessageAvatarProps) => {
	...
	if ((!isReply && isCombine) || (!isReply && !user)) {
		return (
			<div className="w-[38px] flex items-center justify-center min-w-[38px]">
				<div className="hidden group-hover:text-zinc-400 group-hover:text-[10px] group-hover:block">{messageHour}</div>
			</div>
		);
	}
	...
	return (
		<div>
			{hasAvatar ? (
				<img className="w-[38px] h-[38px] rounded-full object-cover min-w-[38px] min-h-[38px]" src={avatarImg} alt={avatarImg} />
			) : (
