...
const MessageWithUser = ({ message, user, isMessNotifyMention, mode, editor, isMention, onContextMenu, isEditing, popup }: MessageWithUserProps) => {
    ...
    const checkReplied = useMemo(() => idMessageRefReply === message.id && openReplyMessageState && message.id !== lastMessageId, [idMessageRefReply, openReplyMessageState, message.id, lastMessageId]);
    const checkMessageTargetToMoved = useMemo(() => idMessageToJump === message.id && message.id !== lastMessageId, [idMessageToJump, message.id, lastMessageId]);
    const hasIncludeMention = useMemo(() => message.content.t?.includes('@here') || message.content.t?.includes(`@${userLogin.userProfile?.user?.username}`), [message.content.t, userLogin.userProfile?.user?.username]);
...
function MessageStatus({ message, isMessNotifyMention }: Partial<MessageWithUserProps>) {
    ...
    return (
        <div className="absolute top-[100] right-2 flex-row items-center gap-x-1 text-xs text-gray-600">
            {shouldShowSentIcon && <Icons.Sent />}
        </div>
    );
}
