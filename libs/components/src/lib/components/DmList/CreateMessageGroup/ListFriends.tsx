...
const ListFriends = forwardRef<HTMLDivElement, ListFriendProps>(({ listFriends, selectedFriends, idActive, handleCheckboxChange }, ref) => {
	return (
				...
				>
					<label className="flex flex-row items-center justify-between w-full gap-2 py-[3px] cursor-pointer">
						<div className="flex flex-row items-center gap-2">
							...
							/>
							<span className={`text-base font-medium dark:text-white text-textLightTheme one-line`}>{friend.user?.display_name}</span>
