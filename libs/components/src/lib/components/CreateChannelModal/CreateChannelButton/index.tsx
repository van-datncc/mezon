...
export const CreateChannelButton: React.FC<CreateChannelProps> = ({ onClickCancel, onClickCreate }) => {
	...
	return (
		<div className="Frame394 relative border-t border-solid border-borderDefault self-stretch mb-0 bg-[#151515] pt-3 justify-end items-center gap-4 inline-flex">
			<button onClick={onClickCancel} className="Text text-white hover:underline text-xs font-medium font-['Manrope'] leading-normal">
				Cancel
			</button>
			...
			<button
				...
				className="Text text-white disabled:cursor-not-allowed text-xs font-medium font-['Manrope'] leading-normal relative h-10 w-30 justify-center px-3 py-3 bg-blue-600 hover:bg-blue-500 rounded-sm flex flex-row items-center gap-1"
			>
