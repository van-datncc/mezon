...
function ModalUnknowChannel(props: ModalUnknowChannelProps) {
	...
	return (
		<div className="w-[100vw] h-[100vh] overflow-hidden fixed top-0 left-0 z-[100] bg-black  flex flex-row justify-center items-center">
			<div className="w-fit h-fit text-theme-primary bg-theme-setting-primary rounded-lg flex-col justify-start  items-start gap-3 inline-flex overflow-hidden">
				<div className={` ${isError ? 'w-[400px]' : ''} `}>
					<div className="p-4 relative">
						<div className="flex flex-col items-center gap-y-3 ">
							<Icons.IconClockChannel />
							{isError ? (
								<h5 className="font-bold text-2xl w-full flex justify-center text-theme-primary">
									{errMessage ? errMessage : 'Oops! Something Went Wrong'}
								</h5>
							) : (
								<>
									<h3 className="font-bold text-2xl ">You don't have access to this link.</h3>
									<p>This link is to a clan or channel you don't have access to.</p>
								</>
							)}
						</div>
						<span
							className="absolute top-3 right-2 text-5xl leading-3  text-theme-primary-hover cursor-pointer"
							onClick={onCloseAndReset}
						>
							×
						</span>
					</div>
					<div className="w-full p-4 border-t-theme-primary">
						{isError ? (
							<button
