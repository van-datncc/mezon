...
import { selectnotificatonSelected, selectDefaultNotificationClan, selectDefaultNotificationCategory } from '@mezon/store';
...
const NotificationSetting = () => {
    ...
    const handleScheduleMute = (duration:number) => {
        // console.log(`Mute notifications for ${duration}`);
		const now = new Date();
        const unmuteTime = new Date(now.getTime() + duration);
        const unmuteTimeISO = unmuteTime.toISOString();
		console.log(`now: ${now}`);
		
		// console.log(`Mute notifications until: ${unmuteTimeISO}`);
        setTimeout(() => {
    ...
    return (
        <div className="absolute top-8 right-0 shadow z-[99999999]">
            <div className="flex flex-col rounded-[4px] w-[202px] shadow-sm overflow-hidden py-[6px] px-[8px] dark:bg-black bg-white">
                ...
                <ItemNotificationSetting children="Use Category Default" type="radio" name='NotificationSetting' defaultNotifi={true} defaultNotifiName={defaultNotifiName} notifiSelected={getNotificationChannelSelected?.notification_setting_type === undefined}/>
                {notificationTypes.map((notification) => (
                    <ItemNotificationSetting children={notification} notificationId={notification} type="radio" name='NotificationSetting' key={notification} notifiSelected={getNotificationChannelSelected?.notification_setting_type === notification} />
                ))}
            </div>
        </div>
    );
