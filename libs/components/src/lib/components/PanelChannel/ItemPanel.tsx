...
const ItemPanel = ({ children, dropdown, danger, type, onClick }: ItemPanelProps) => {
	return (
		<button onClick={onClick} className="flex items-center w-full justify-between rounded-sm hover:bg-bgSelectItem hover:[&>*]:text-[#fff] pr-2">
			<li className={`text-[14px] ${danger ? 'text-colorDanger' : 'dark:text-[#B5BAC1] text-colorTextLightMode'} font-medium w-full py-[6px] px-[8px] text-left cursor-pointer list-none textWhiteHoverImportant m-0`}>{children}</li>
