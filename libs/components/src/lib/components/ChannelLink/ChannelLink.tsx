...
const ChannelLinkComponent = React.forwardRef<ChannelLinkRef, ChannelLinkProps>(
	(
		...
		const isNotChannelVoiceOrApp = useMemo(
		...
		return (
			<div
				...
				role="button"
				className={`relative group ${(isUnReadChannel && isNotChannelVoiceOrApp) || (numberNotification && numberNotification > 0 && isNotChannelVoiceOrApp) ? 'before:content-[""] before:w-1 before:h-2 before:rounded-[0px_4px_4px_0px] before:absolute dark:before:bg-channelActiveColor before:bg-channelActiveLightColor before:top-3' : ''}`}
			>
				{channelType === ChannelType.CHANNEL_TYPE_VOICE ? (
					...
					>
						...
						<p
							className={`ml-2 w-full dark:group-hover:text-white group-hover:text-black text-base focus:bg-bgModifierHover ${(isActive && isNotChannelVoiceOrApp) || (isUnReadChannel && isNotChannelVoiceOrApp) || (numberNotification && numberNotification > 0 && isNotChannelVoiceOrApp) ? 'dark:text-white text-black dark:font-medium font-semibold' : 'font-medium dark:text-channelTextLabel text-colorTextLightMode'}`}
