...
const SettingSoundEffect = () => {
	...
	return (
		<div className="flex flex-col gap-6 pb-[40px]  text-sm">
			...
			<div
				className="flex p-4 
             rounded-lg shadow-sm hover:shadow-md transition duration-200 border-theme-primary"
			>
			...
			<div className="flex flex-col gap-4">
				...
				<div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 ">
					{soundList.length === 0 && (
						<div className="col-span-full flex flex-col items-center justify-center py-10 border-dashed rounded-lg border-2 border-color-primary text-center">
							<Icons.Speaker className="w-10 h-10 mb-2" />
							<p className="text-sm">No sound effects yet. Click "Upload sound" to add!</p>
						</div>
					)}
					{soundList.map((sound) => (
						<div key={sound.id} className="flex flex-col w-full p-4 border rounded-lg shadow-sm hover:shadow-md transition duration-200 ">
