...
export const WebRTCProvider: React.FC<WebRTCProviderProps> = ({ children }) => {
	...
	const initializePeerConnection = useCallback(() => {
		...
		peerConnection.current.onicecandidate = async (event) => {
			if (event && event.candidate && mezon.socketRef.current?.isOpen() === true) {
				await mezon.socketRef.current?.SFUSignalingFwd(
					clanId.current || '',
					channelId.current || '',
	...
	const startLocalStream = async () => {
		try {
			...
			await mezon.socketRef.current?.SFUSignalingFwd(clanId.current || '', channelId.current || '', WebrtcSignalingType.WEBRTC_SDP_INIT, '');
		} catch (error) {
