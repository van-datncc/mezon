...
import type { ApiRole, ApiRoleListEventResponse, ApiUpdateRoleOrderRequest, ApiUpdateRoleRequest, RoleUserListRoleUser } from 'mezon-js';
...
export const fetchRolesClanCached = async (getState: () => RootState, ensuredMezon: MezonValueContext, clanId: string, noCache = false) => {
	...
	const response = (await fetchDataWithSocketFallback(
		...
		(session) => ensuredMezon.client.listRoles(session, clanId, 500, 1, ''),
		'role_event_list'
...
export const fetchMembersRole = createAsyncThunk('MembersRole/fetchMembersRole', async ({ roleId, clanId }: FetchMembersRolePayload, thunkAPI) => {
	try {
		const mezon = await ensureSession(getMezonCtx(thunkAPI));
		const response = await withRetry((session) => mezon.client.listRoleUsers(session, roleId, 100, ''), {
			maxRetries: 3,
			initialDelay: 1000,
