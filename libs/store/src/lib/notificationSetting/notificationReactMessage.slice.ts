...
export const setNotifiReactMessage = createAsyncThunk(
  ...
  async ({ channel_id }: SetNotifiReactMessagePayload, thunkAPI) => {
    const mezon = await ensureSession(getMezonCtx(thunkAPI));
    const response = await mezon.client.setNotificationReactMessage(
      mezon.session,
      channel_id || "",
    );
    if (!response) {
    ...
    thunkAPI.dispatch(
      getNotifiReactMessage({ channelId: channel_id || "", noCache: true }),
    );
    return response;
  },
);
...
export const deleteNotifiReactMessage = createAsyncThunk(
  ...
  async ({ channel_id }: DeleteNotifiReactMessagePayload, thunkAPI) => {
    const mezon = await ensureSession(getMezonCtx(thunkAPI));
    const response = await mezon.client.deleteNotiReactMessage(
      mezon.session,
      channel_id || "",
    );
    if (!response) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (
          state: NotifiReactMessageState,
          action: PayloadAction<ApiNotifiReactMessage>,
        ) => {
      ...
      .addCase(
        getNotifiReactMessage.rejected,
        (state: NotifiReactMessageState, action) => {
...
export const getNotifiReactMessageState = (rootState: {
  [NOTIFI_REACT_MESSAGE_FEATURE_KEY]: NotifiReactMessageState;
}): NotifiReactMessageState => rootState[NOTIFI_REACT_MESSAGE_FEATURE_KEY];

export const selectNotifiReactMessage = createSelector(
  getNotifiReactMessageState,
  (state: NotifiReactMessageState) => state.notifiReactMessage,
);
