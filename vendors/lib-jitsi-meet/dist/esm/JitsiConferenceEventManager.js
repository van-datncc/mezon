...
JitsiConferenceEventManager.prototype.setupChatRoomListeners = function () {
    ...
    this.chatRoomForwarder.forward(XMPPEvents.MEETING_ID_SET, JitsiConferenceEvents.CONFERENCE_UNIQUE_ID_SET);
    this.chatRoomForwarder.forward(XMPPEvents.CONFERENCE_TIMESTAMP_RECEIVED, JitsiConferenceEvents.CONFERENCE_CREATED_TIMESTAMP);
    ...
    chatRoom.addListener(XMPPEvents.RECORDER_STATE_CHANGED, (session, jid) => {
        ...
        conference.eventEmitter.emit(JitsiConferenceEvents.RECORDER_STATE_CHANGED, session);
    });
    this.chatRoomForwarder.forward(XMPPEvents.TRANSCRIPTION_STATUS_CHANGED, JitsiConferenceEvents.TRANSCRIPTION_STATUS_CHANGED);
    this.chatRoomForwarder.forward(XMPPEvents.VIDEO_SIP_GW_AVAILABILITY_CHANGED, JitsiConferenceEvents.VIDEO_SIP_GW_AVAILABILITY_CHANGED);
    this.chatRoomForwarder.forward(XMPPEvents.VIDEO_SIP_GW_SESSION_STATE_CHANGED, JitsiConferenceEvents.VIDEO_SIP_GW_SESSION_STATE_CHANGED);
    ...
    this.chatRoomForwarder.forward(XMPPEvents.MUC_MEMBERS_ONLY_CHANGED, JitsiConferenceEvents.MEMBERS_ONLY_CHANGED);
    this.chatRoomForwarder.forward(XMPPEvents.MUC_VISITORS_SUPPORTED_CHANGED, JitsiConferenceEvents.VISITORS_SUPPORTED_CHANGED);
    chatRoom.addListener(XMPPEvents.MUC_MEMBER_JOINED, conference.onMemberJoined.bind(conference));
    this.chatRoomForwarder.forward(XMPPEvents.MUC_LOBBY_MEMBER_JOINED, JitsiConferenceEvents.LOBBY_USER_JOINED);
    this.chatRoomForwarder.forward(XMPPEvents.MUC_LOBBY_MEMBER_UPDATED, JitsiConferenceEvents.LOBBY_USER_UPDATED);
    this.chatRoomForwarder.forward(XMPPEvents.MUC_LOBBY_MEMBER_LEFT, JitsiConferenceEvents.LOBBY_USER_LEFT);
    ...
    this.chatRoomForwarder.forward(XMPPEvents.MUC_DENIED_ACCESS, JitsiConferenceEvents.CONFERENCE_FAILED, JitsiConferenceErrors.CONFERENCE_ACCESS_DENIED);
    ...
    (jid, txt, myJid, ts, nick, isGuest) => {
        ...
        conference.eventEmitter.emit(JitsiConferenceEvents.MESSAGE_RECEIVED, id, txt, ts, nick, isGuest);
    });
    chatRoom.addListener(XMPPEvents.PRIVATE_MESSAGE_RECEIVED, 
    // eslint-disable-next-line max-params
    (jid, txt, myJid, ts) => {
        const id = Strophe.getResourceFromJid(jid);
        conference.eventEmitter.emit(JitsiConferenceEvents.PRIVATE_MESSAGE_RECEIVED, id, txt, ts);
    });
    ...
    chatRoom.addListener(XMPPEvents.JSON_MESSAGE_RECEIVED, (from, payload) => {
        ...
        if (participant) {
            ...
            conference.eventEmitter.emit(JitsiConferenceEvents.NON_PARTICIPANT_MESSAGE_RECEIVED, id, payload);
        }
    });
...
JitsiConferenceEventManager.prototype.setupRTCListeners = function () {
    ...
    rtc.addListener(RTCEvents.DOMINANT_SPEAKER_CHANGED, (dominant, previous, silence) => {
        if ((conference.lastDominantSpeaker !== dominant || conference.dominantSpeakerIsSilent !== silence)
            ...
            conference.eventEmitter.emit(JitsiConferenceEvents.DOMINANT_SPEAKER_CHANGED, dominant, previous, silence);
...
JitsiConferenceEventManager.prototype.setupXMPPListeners = function () {
    ...
    this._addConferenceXMPPListener(XMPPEvents.AV_MODERATION_APPROVED, value => conference.eventEmitter.emit(JitsiConferenceEvents.AV_MODERATION_APPROVED, { mediaType: value }));
    ...
    this._addConferenceXMPPListener(XMPPEvents.VISITORS_MESSAGE, value => conference.eventEmitter.emit(JitsiConferenceEvents.VISITORS_MESSAGE, value));
    this._addConferenceXMPPListener(XMPPEvents.VISITORS_REJECTION, () => conference.eventEmitter.emit(JitsiConferenceEvents.VISITORS_REJECTION));
};
/**
